...
( SELECT  nodeA, nodeB, weight AS path_length, 0, ARRAY[]::varchar[] || nodeA || nodeB AS path
  FROM g1_edges_subset WHERE nodeA = 'id1' 
 
  UNION ALL
 
  SELECT tc.nodeA, e.nodeB, tc.weight + e.weight, depth + 1, path || e.nodeB
  FROM g1_edges_subset AS e, search_graph AS tc 
  WHERE e.nodeA = tc.nodeB AND e.nodeB <> ALL(path)
)
SELECT * FROM search_graph ORDER BY depth) AS temp WHERE nodeA = 'id1' AND nodeB='id2' AND weight>=3) AS temp2;

CREATE TABLE g1_edges_subset(
nodeA VARCHAR(10) NOT NULL,
nodeB VARCHAR(10) NOT NULL,
weight INTEGER DEFAULT 0,
PRIMARY KEY(nodeA, nodeB)
);

INSERT INTO g1_edges_subset VALUES 
('id1', 'id2', 3), 
('id1', 'id7', 1),
('id1', 'id5', 1),
('id1', 'id6', 1),
